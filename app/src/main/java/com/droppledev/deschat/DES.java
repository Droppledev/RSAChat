...
public class DES {
    public static String encrypt(String key, String msg){
        try {
            ...
            byte[] theMsg = msg.getBytes();
            byte[] theCph = cipher(theMsg, subKeys, "encrypt");
            String hasil = Arrays.toString(theCph);
            hasil = hasil.replace("[","");
            hasil = hasil.replace("]","");
            return hasil;
        } catch (Exception e) {
    ...
    public static String decrypt(String key, String msg){
        try {
            byte[] theKey = key.getBytes();
            byte[][] subKeys = getSubkeys(theKey);
            byte[] theMsg = stringToByteArray(msg);
            byte[] theCph = cipher(theMsg, subKeys, "decrypt");
    ...
    public static byte[] stringToByteArray(String msg){
        List<String> stringMsg = Arrays.asList(msg.split("\\s*,\\s*"));
        byte[] byteMsg = new byte[stringMsg.size()];
        for (int i = 0;i<stringMsg.size();i++){
            byteMsg[i] = (byte) Integer.parseInt(stringMsg.get(i));
        }
        return byteMsg;
    }
    ...
    static final int[] INVP = { 40, 8, 48, 16, 56, 24, 64, 32, 39, 7, 47, 15, 55, 23, 63, 31, 38, 6, 46, 14, 54, 22, 62,
            30, 37, 5, 45, 13, 53, 21, 61, 29, 36, 4, 44, 12, 52, 20, 60, 28, 35, 3, 43, 11, 51, 19, 59, 27, 34, 2, 42,
            10, 50, 18, 58, 26, 33, 1, 41, 9, 49, 17, 57, 25 };

    public static byte[] fillWithZero(byte[] a) {
        byte[] out = new byte[8];
        for (int k = 0; k < 8; k++) {
            if (k < a.length) {
                out[k] = a[k];
    ...
    private static byte[][] splitBlocks(byte[] in, int len) {
        int numOfBlocks = ((in.length - 1) / len) + 1;
        int numOfbits = 0;
        byte[] out[] = new byte[numOfBlocks][len];
        for (int i = 0; i < numOfBlocks; i++) {
            for (int j = 0; j < len; j++) {
                if (numOfbits < in.length)
                    out[i][j] = in[numOfbits];
                else
                    out[i][j] = 0;
                numOfbits++;
            }
    ...
    public static byte[] cipher(byte[] theMsg, byte[][] subKeys, String mode) throws Exception {
        byte[] lr = new byte[theMsg.length];
        byte[] out = new byte[0];
        byte blocks[][] = splitBlocks(theMsg, 8);
        for (byte a[] : blocks) {
            theMsg = a;
            ...
            lr = concatenateBits(r, blockSize / 2, l, blockSize / 2);
            lr = selectBits(lr, INVP); // Inverse Permutation
            out = concatArray(out, lr);
        }
        return out;
    }
    ...
    public static byte[][] getSubkeys(byte[] theKey) throws Exception {
        if (theKey.length > 8)
            throw new Exception("Kunci Plaintext tidak boleh lebih dari 8 karakter");
        else
            theKey = fillWithZero(theKey);
